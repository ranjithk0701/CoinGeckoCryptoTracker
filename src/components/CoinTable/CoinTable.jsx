...
function CoinTable() {
    ...
    const { curreny } = useContext(CurrencyContext);
    const [page, setPage] = useState(1);

    const { data, isLoading, isError, error } = useQuery({
        queryKey: ['coins', page, curreny],
        queryFn: () => fetchCoinData(page, curreny),
        // retry: 2,
        // retryDelay: 1000,
        cacheTime: 1000 * 60 * 2,
        staleTime: 1000 * 60 * 2,
                ...
                {data && data.map((coin) => {
                    return (
                        <div key={coin.id} className="w-full bg-transparent text-white flex py-4 px-2 font-semibold items-center justify-between">
                            <div className=" flex items-center justity-start gap-3 basis-[35%] ">
